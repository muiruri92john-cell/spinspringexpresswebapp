...
app.post('/register-device', isOwner, async (req, res) => {
  try {
...
app.post('/owner/attendants', isOwner, async (req, res) => {
  try {
    ...
    const [existing] = await db.query('SELECT id FROM ss_attendants WHERE email = ? AND owner_id = ?', [email, req.session.spinUser.id]);
